...
impl Create {
    ...
    fn _already_exists_err_msg(&self, err: &io::Error) -> () {
        eprintln!(
            "{} {}\n",
            self.name.display(),
            self._colour(9).apply_to("already exists, please pick a different name")
        );
    ...
    fn _permission_denied_err_msg(&self, err: &io::Error) -> () {
        eprintln!(
            "{} {} {}",
            self._colour(9).apply_to("You don't have permission to create"),
            self.name.display(),
            self._colour(9).apply_to(
    ...
    fn _generic_err_msg(&self, err: &io::Error) -> () {
        eprintln!(
            ...
            self._colour(12).apply_to("https://github.com/stats-student/geoffrey-rs/issues")
        );
        panic!("{:?}", err);
    }
            ...
            Err(err) => {
                match err.kind() {
                    io::ErrorKind::AlreadyExists => {
                        self._already_exists_err_msg(&err)
                    }, // AlreadyExists
                    io::ErrorKind::NotFound => {
                        self._not_found_err_msg(&err)
                    }, // NotFound
                    io::ErrorKind::PermissionDenied => {
                        self._permission_denied_err_msg(&err)
                    }
                    _ => {
    ...
    fn create_root(&self) -> () {
        ...
        if self.parents {
            result = create_dir_all(&self.name);
        } else {
            result = create_dir(&self.name);
        }

        self._validate_create_result(&result);
    } // fn create_root

    fn create_subdirectories(&self) -> () {
        let subdirs = vec![
            "data_sources", "explorations", "models", "products"
        ];

        for subdir in subdirs.iter() {
            let full_subdir = &format!("{}/{}/", self.name.display(), subdir);
            create_dir(full_subdir).expect(&format!("Unable to create {}", full_subdir));
        }
    }

    fn create_files(&self) -> () {
        let files = vec![
            "README.md", "project_scoping.md", ".geoff"
        ];

        for file in files.iter() {
            let full_file_path = &format!("{}/{}", self.name.display(), file);
            write(full_file_path, "").expect(&format!("Unable to create {}", full_file_path));
        }
    }

    fn create_tree(&self) -> StringItem {
        let bold = Style::new().bold();

        let tree = TreeBuilder::new(format!("{}", bold.apply_to("test_project")))
            .add_empty_child(
                format!("{} data_sources", self._colour(220).apply_to("\u{1F5BF}"))
            )
            .add_empty_child(
                format!("{} explorations", self._colour(220).apply_to("\u{1F5BF}"))
            )
            .add_empty_child(
                format!("{} models", self._colour(220).apply_to("\u{1F5BF}"))
            )
            .add_empty_child(
                format!("{} products", self._colour(220).apply_to("\u{1F5BF}"))
            )
            .add_empty_child(
                format!("{} README.md", self._colour(194).apply_to("\u{1F5CE}"))
            )
            .add_empty_child(
                format!("{} project_scoping.md", self._colour(194).apply_to("\u{1F5CE}"))
            )
            .build();
